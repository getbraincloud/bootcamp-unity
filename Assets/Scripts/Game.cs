...
public class Game : MonoBehaviour
{
    ...
    public void HandleAuthentication()
    {
        ...
        if (Network.sharedInstance.HasAuthenticatedPreviously())
        {
            ...
            Network.sharedInstance.Reconnect(OnAuthenticationRequestCompleted, OnAuthenticationRequestFailed);
        }
        else
        {
    ...
    public void OnAuthenticationRequestCompleted(ref AuthenticationData authenticationData)
    {
        if (m_GameState == GameState.Authenticating)
        {
            ...
            if (authenticationData.MainLeaderboard != null)
                LeaderboardsManager.sharedInstance.AddLeaderboard(authenticationData.MainLeaderboard);
            else
            ...
            if (authenticationData.DailyLeaderboard != null)
                LeaderboardsManager.sharedInstance.AddLeaderboard(authenticationData.DailyLeaderboard);
            else
            ...
            if (authenticationData.CountryLeaderboard != null)
                LeaderboardsManager.sharedInstance.AddLeaderboard(authenticationData.CountryLeaderboard);
            else
                Network.sharedInstance.RequestCountryLeaderboard(OnLeaderboardRequestCompleted);


            //Set the Statistics
            if (authenticationData.StatisticsList != null)
            {
                List<Statistic> statisticsList = authenticationData.StatisticsList;
                StatisticsManager.sharedInstance.SetStatistics(ref statisticsList);
            }
            else
            ...
            {
                List<Achievement> achievementsList = authenticationData.AchievementsList;
                AchievementManager.sharedInstance.SetAchievements(ref achievementsList);
            }
            else
                Network.sharedInstance.RequestReadAchievements(OnAchievementRequestCompleted);

            //Set the UserData
            if (authenticationData.UserProgress != null)
                m_UserData = authenticationData.UserProgress;
            else
    ...
    public void OnAuthenticationRequestFailed(string errorMessage)
    {
        if (m_GameState == GameState.Authenticating)
        {
            ...
            Network.sharedInstance.ResetStoredProfileId();
        }
    }
    ...
    private void NextLevel()
    {
        ...
        if (needsSync)
            Network.sharedInstance.UpdateUserEntityData(m_UserData.EntityID, m_UserData.EntityType, m_UserData.JsonData);
